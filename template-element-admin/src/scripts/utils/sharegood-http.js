...
;(function(global, factory) {
    ...
    var encode$1 = function encode(str, defaultEncoder, charset, kind, format) {
        ...
        for (var i = 0; i < string.length; ++i) {
            ...
            if (c < 0xd800 || c >= 0xe000) {
                out =
                    out +
                    (hexTable[0xe0 | (c >> 12)] + hexTable[0x80 | ((c >> 6) & 0x3f)] + hexTable[0x80 | (c & 0x3f)])
                continue
            }
    ...
    var normalizeStringifyOptions = function normalizeStringifyOptions(opts) {
        ...
        return {
            ...
            encodeValuesOnly:
                typeof opts.encodeValuesOnly === 'boolean' ? opts.encodeValuesOnly : defaults$1.encodeValuesOnly,
            filter: filter,
            format: format,
            formatter: formatter,
    ...
    var toFormData = function toFormData(data) {
        ...
        lodash.each(data, function(val, key) {
    ...
    function wrapAxios(instance) {
        ...
        lodash.each(instance, function(val, key) {
            wrap[key] = val
        })
        ...
        wrap.exHooks.add = function(obj) {
            ...
            var _id = lodash.uniqueId(''.concat(Date.now()))
    ...
    var setConfig = function setConfig(config) {
        ...
        if (cancelToken instanceof CancelToken$1) {
            token = cancelToken
            _promise = cancelToken.promise
        } else {
            token = new CancelToken$1(lodash.noop)
        }
        ...
        token._exCancel_id = lodash.uniqueId(''.concat(Date.now()))
    ...
    var hooks = Object.freeze({
        onBefore: function onBefore(config) {
            ...
            if (config.exCancel) {
                ...
                names = lodash.union(names).map(function(name) {
                    if (name === true)
                        return ''
                            .concat(method)
                            .concat(baseURL)
    ...
    var cancel = function cancel(name) {
        ...
        lodash.each(tokens, function(val, key) {
            if (names.includes(key)) {
                val.forEach(function(token) {
                    if (token.reason) return
                    token.reason = new Cancel$1('exCancel')
    ...
    var normalizeParseOptions = function normalizeParseOptions(opts) {
        ...
        return {
            ...
            allowPrototypes:
                typeof opts.allowPrototypes === 'boolean' ? opts.allowPrototypes : defaults$2.allowPrototypes,
            arrayLimit: typeof opts.arrayLimit === 'number' ? opts.arrayLimit : defaults$2.arrayLimit,
            charset: charset,
            charsetSentinel:
                typeof opts.charsetSentinel === 'boolean' ? opts.charsetSentinel : defaults$2.charsetSentinel,
            ...
            delimiter:
                typeof opts.delimiter === 'string' || utils$1.isRegExp(opts.delimiter)
                    ? opts.delimiter
            ...
            strictNullHandling:
                typeof opts.strictNullHandling === 'boolean' ? opts.strictNullHandling : defaults$2.strictNullHandling
        }
    }
    ...
    var createAxios = function createAxios(requestConfig, callback) {
        var _requestConfig$isResp = requestConfig.isResponseSuccess,
            ...
            rest = _objectWithoutProperties(requestConfig, [
                'isResponseSuccess',
                'getResponseError',
                'getResponseSuccess'
            ])
        ...
        var _axios = axios$1.create(mergeConfig(defaults, rest))
